...
#
# Всё тело — одна фигурная группа с `exit` в конце. Это не украшение:
# скрипт обновляет сам себя через git pull, а bash дочитывает файл по мере
# выполнения. Без группы он после обновления продолжил бы читать НОВЫЙ файл
# со старого смещения и выполнил бы обрывок строки. Фигурная группа
# заставляет разобрать весь файл до первой команды.
{
set -euo pipefail
...
AFTER=$(sha1sum "$0" | cut -d" " -f1)

# Продолжать старой версией нельзя: именно так исправленная ошибка
# воспроизводилась ещё раз, хотя починка уже лежала на диске.
if [ "$BEFORE" != "$AFTER" ] && [ "${LINGUICH_REEXEC:-}" != "1" ]; then
    echo "==> Скрипт выкатки обновился, перезапускаю его новой версией"
    LINGUICH_REEXEC=1 exec "$0" "$@"
fi
...
# Читаем через env_value, а не подключением файла: значения там не
# экранированы под шелл, и SECRET_KEY со скобками обрывал выкатку до миграций.
